...
class PlotComponent extends JComponent {
	...
	protected void paintComponent(Graphics g) {
		...
		for (int i = 0; i < n + 1; i++) y_i[i] = f.evaluate(a + i * 1.0 * (b - a) / n);
		this.method.init(a, b, n, y_i);

		minX = a; maxX = b;
		for (int i = 0; i < numOfPoints; i++) xData[i] = minX + i * (maxX - minX) / (numOfPoints - 1);
		...
		for (int i = 0; i < numOfPoints; i++) {
			if (fData[i] < minY) minY = fData[i];
			if (fData[i] > maxY) maxY = fData[i];
		}
